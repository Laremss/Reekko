...
const PROBLEMSECTION_DEFAULTS = {
  ...
  transitionText: 'Et si votre pipeline se remplissait chaque semaine',
  transitionHighlight: 'sans que votre équipe ait à prospecter manuellement ?',
}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <AnimateOnScroll delay={150}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {problems.map((problem, index) => {
              const iconName = Object.keys(iconMap)[index] as keyof typeof iconMap
        ...
        <AnimateOnScroll delay={300} className="mt-20 text-center">
          <p className="text-zinc-400 text-base">
            {transitionText}{' '}
            <span className="text-white font-medium">{transitionHighlight}</span>
          </p>
        </AnimateOnScroll>
      </div>
    </section>
